Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", path: "./scripts/scriptinstall.sh"
  config.vm.define "server" do |server|
	server.vm.hostname = "server"
	server.vm.provision "shell", path: "./scripts/scriptserver.sh"
	server.vm.network "private_network", ip: "192.168.1.2", virtualbox__intnet: "mynetwork"
  end

  config.vm.define "agent1" do |agent1|
	agent1.vm.hostname = "agent1"
	agent1.vm.provision "shell", path: "./scripts/scriptagent.sh"
	agent1.vm.network "private_network", ip: "192.168.1.3", virtualbox__intnet: "mynetwork"
  end
  
  config.vm.define "agent2" do |agent2|
	agent2.vm.hostname = "agent2"
	agent2.vm.provision "shell", path: "./scripts/scriptagent2.sh"
